...
$resultk = $mysqli->query("SELECT * FROM `resource_level`  ORDER BY  `id` ASC ");
	...
	//$created_at=date("Y-m-d H:i:s");
		...
		//$hint=$_REQUEST['hint'];
			...
			$sql_con="INSERT INTO `prob_ans` SET 
							`question_id`='$fetchlevelval',
							`text`='$fetchans'";  
			$res=$mysqli->query($sql_con);	 
			if($res)	
			{
				@header("Location: proxy_mgt.php?msg=Sucessful Insert");
				exit();
			}			
	}
...
<input type="hidden"  name="mode" id="mode" value="1" >					
		 ...
		 <tr class="mysty2" bgcolor="#A9B63D" height="18">
    				<td colspan="2" class="mysty2" align="center" bgcolor="#000000"><span class="style2"><strong><font color="#ffffff">Add Answer</font></strong></span></td>
		  </tr>
...
<td width="332">
<select name="level_id"  id="level_id" style="width:195px" onChange="window.location.href='add_proxans.php?level='+this.value"  > 
